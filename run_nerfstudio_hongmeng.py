...
def parse_args():
    ...
    parser.add_argument("--vis_online", action="store_true", help="Whether to visualize online")
    parser.add_argument("--random_init", action="store_true", help="Whether to use random initialization")
...
def main():
    ...
    crop_reference_ply = os.path.join(data_dir,"sparse/0/points3D.ply")
    ...
    try:
        ...
        exit(1)
    # 裁剪
    # if not os.path.exists(crop_reference_ply):
    if True:
        splat_output_ply_dir = os.path.dirname(splat_output_ply)
        os.makedirs(splat_output_ply_dir, exist_ok=True)
        shutil.copy2(crop_input_ply, crop_output_ply)
