...
from nerfstudio.utils.misc import step_check, step_iteration_save, crop_gs, convert_ply_to_splat
from nerfstudio.utils.rich_utils import CONSOLE
from nerfstudio.utils.writer import EventName, TimeWriter
from nerfstudio.viewer.viewer import Viewer as ViewerState
from nerfstudio.viewer_legacy.server.viewer_state import ViewerLegacyState
from nerfstudio.utils.eval_utils import export_gs
import math
...
class Trainer:
    ...
    def __init__(self, config: TrainerConfig, local_rank: int = 0, world_size: int = 1) -> None:
        ...
        self.get_save_iteration_output_dir: Path = config.get_save_iteration_output_dir()
    ...
    def train(self) -> None:
        ...
        with TimeWriter(writer, EventName.TOTAL_TRAIN_TIME):
            ...
            for step in range(self._start_step, self._start_step + num_iterations):
                ...
                # 添加进度
                if self.step % 1000 == 0 or self.step == self.config.max_num_iterations - 1:  # 每 1000 步保存一次
                    progress_ratio = math.ceil((self.step / self.config.max_num_iterations) * 100)  # 计算完成百分比
                ...
                if step_iteration_save(step, self.config.save_iteration):
                    # print("+++++++++++++++++++++++++++test")
                    self.save_checkpoint(step)
                    outputexport_dir = os.path.join(self.get_save_iteration_output_dir,"exports")
                    output_dir = self.get_save_iteration_output_dir
                    load_config = Path(os.path.dirname(self.checkpoint_dir)) / "config.yml"
                    output_filename = "splat.ply"
                    # 导出 export 文件夹中
                    export_gs(outputexport_dir, load_config, output_filename, self.config.save_iteration)
                    # crop_gs f"iteration_"
                    output_ply = os.path.join(output_dir,f"point_cloud/iteration_{self.config.save_iteration}" ,"point_cloud.ply")
                    output_ply_parent = os.path.dirname(output_ply)
                    if not os.path.exists(output_ply_parent):
                        os.makedirs(output_ply_parent, exist_ok=True)
                    reference_ply = os.path.join(str(self.checkpoint_dir.parent.parent), "fastRecon/dense/sparse/0","points3D.ply")
                    # if os.path.exists(reference_ply):
                    if True:
                        # print(f"Reference PLY file found at {reference_ply}.")
                        print("directory exists, crop gs")
                        splat_output_ply = os.path.join(output_dir,f"point_cloud/iteration_{self.config.save_iteration}" ,"splat.splat")
                        input_ply = os.path.join(outputexport_dir, output_filename)
                        shutil.copy2(input_ply, output_ply)
                        convert_ply_to_splat(input_ply, splat_output_ply)
                    # else:
                    #     scale_factor = 10
                    #     input_ply = os.path.join(outputexport_dir, output_filename)
                    #     max_sh_degree = 0
                    #     crop_gs(output_ply, reference_ply, scale_factor, input_ply, max_sh_degree)
                    #     crop_output_ply = output_ply
                    #     splat_output_ply = os.path.join(output_dir,f"point_cloud/iteration_{self.config.save_iteration}" ,"splat.splat")
